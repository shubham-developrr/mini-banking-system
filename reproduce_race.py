...
# Use a hardcoded user for testing
TEST_EMAIL = f"test_race_{int(time.time())}@example.com"
...
def run_race_condition_test():
    ...
    if not user_data:
        print("Skipping test due to registration failure (server might be down)")
        return
    ...
    # Expected: 10 success, 5 fail.
    # Actual (with race condition): Likely all 15 success, balance becomes negative.
    
    threads = []
    results = []

    def perform_withdrawal():
        # Each thread needs its own session to simulate real concurrent users? 
        # Actually, flask session is cookie based. If we share session object, requests lib handles cookies.
        # But race condition is on server DB access, not session.
        # So sharing session is fine, or creating new ones.
        # Let's use the same session to simulate "double click" or parallel requests from same user.
    ...
    if balance < 0:
        print("❌ CRITICAL FAIL: Balance is negative! Race condition confirmed.")
